...
void PointerExtras::SetTapPositioning(bool on) {
    std::lock_guard<std::mutex> lk(mutex_);
    tapPositioning_ = on;
    OH_LOG_INFO(LOG_APP, "[PtrExt] tapPositioning=%{public}d", on ? 1 : 0);
}

bool PointerExtras::TapPositioning() const {
    std::lock_guard<std::mutex> lk(mutex_);
    return tapPositioning_;
}
