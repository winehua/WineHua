...
**run 长时间无结果，设备上 App 停在首页（Wine 环境「未启动」）**

重建过 HAP（引擎内容变了）时，设备端要你确认「应用引擎更新（重置）」才解压新引擎，
确认前引擎不会启动、测试自然不会跑。这是**不可恢复的容器重置**（清空 prefix），
需要人在设备上点确认 —— 本工具不代点；点完重跑即可（`C:\smoke` 由 run 重新推送）。
