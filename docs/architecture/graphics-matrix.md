...
```
机型 == VYG-AL00 且 系统增量版本 == 26.0.0.32  →  dxvk_modern_2_6
其他                                          →  dxvk_legacy
```
...
| D3D 版本 | 谁处理 | 说明 |
|---|---|---|
| **D3D12** | vkd3d-proton | 仅 `vkd3d_limited_500k` 档。其他档会走 Wine 自带实现，而它找不到 Vulkan 驱动（`vkCreateInstance` 失败） |
...
| **D3D9** | Wine 自带 wined3d | DXVK 虽然能构建出 d3d9，但打包时不带它，所以 D3D9 始终走 OpenGL 通道 |
...
3. **`no_semaphore_feedback` 必须恒定携带**，不能跟着 DXVK 档位派生。曾经漏掉它导致 vtest 卡在信号量等待、D3D12 白屏；而且"先切 2.6 再切回"的残留档位会掩盖这个 bug。
...
8. **ARM64X 的结论**：DXVK 系列可以做真双架构，vkd3d **已放弃**（手工脚本产出的"双架构"其实是假的，真 ARM64EC 在转译层会崩），所以 d3d12 只有 x86_64 单架构版本。
