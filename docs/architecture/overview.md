...
- **wine_child 是 Guest 域的容器**：box64 以共享库形式 dlopen 进 wine_child，`box64_hmos_main()` 在同一进程内模拟执行 x86_64 Wine ELF（见 [wine-internals.md](wine-internals.md) §2）。
...
- **音频 Host Broker 在 App 主进程内**：Wine 子进程不直接创建设备，宿主 native 进程独占 `OH_AudioRenderer`（设计原则见 [audio.md](audio.md)）。

## 3. 图形链路

两条链路，DXVK/Venus 为产品主路径，OpenGL 为显式 fallback（详见 [opengl-virgl.md](opengl-virgl.md)、[PHASE2_DXVK_STATUS_MEMO.md](../archive/PHASE2_DXVK_STATUS_MEMO.md)）。
...
- 环境变量契约（`WINEHUA_GRAPHICS_BACKEND`、`WINEHUA_FRAME_TRANSPORT`、`VTEST_SOCKET_NAME` 等）以 [opengl-virgl.md](opengl-virgl.md) §宿主 ↔ guest 环境变量契约为准。
...
- 状态（z-order、toplevel 聚合）收在 `compositor/` 的 owning classes（各头注释声明不变式）。协议层尚未瘦身为纯解析：`xdg_shell.cpp` 基本只管协议；`wl_core.cpp` 除 wl_compositor/surface/subcompositor/subsurface/viewporter 协议实现外，surface_commit 路径仍混有 SHM 上传/图像处理、窗口管理策略（位置同步、尺寸漂移重发 configure、root 识别）、popup 管理等协议外职责（收敛计划见 [COMPOSITOR_REFACTOR_PLAN.md](../archive/COMPOSITOR_REFACTOR_PLAN.md) 阶段 5）。
...
- **控制面/数据面分离**：两级 fd 协议（bootstrap fd → private socketpair），命令走 IPC，PCM 走共享内存 ring；callback 只做取数、混音、补零，不做阻塞操作（详见 [audio.md](audio.md)）。
...
| [wine-internals.md](wine-internals.md) | Wine 内部 PE/Unix 分层、compositor 模块结构、信号/IO 细节 | 改 Wine 或 compositor 前 |
| [opengl-virgl.md](opengl-virgl.md) | VirGL/OpenGL 链路设计 + `WINEHUA_*` 环境变量契约 | 改图形环境变量或 GL 链路 |
