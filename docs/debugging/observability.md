...
合成器内部靠**消息前缀**再细分（因为协议层统一用 `WL_Server` 标签）：`[MW-RNDR]` 渲染、`[MW-TAKE]` 取帧、`[MW-SUBSURF]` 子表面、`[MW-MOVE]` 窗口拖动、`[MW-GEO]` / `[MW-RESIZE]` 几何与尺寸映射、`[MW-POPUP]` 弹出层、`[MW-COMMIT]` 帧提交、`[MW-NAPI]` 界面层转发。渲染循环内还有三个细分前缀：`DRAW-TIME`（实测 surface 尺寸与 ArkTS 声明尺寸不符的告警，说明声明没被系统采纳；数值变化才打）、`re-letterbox`（无新帧但尺寸与上次实际绘制的尺寸不符 → 强制重绘，静态桌面跳帧分支的例外路径）、`DBG-FIT`（几何变化时的 surface/frame/letterbox 状态快照，变化才打）。
