...
app.hackeris.winehua HAP 在 NAPI 沙箱中启动 Wine，dosdevices symlink 不可用已全覆盖修复。
...
- XKB keymap 正确打包到 HNP (`XKB_CONFIG_ROOT` 指向 `/opt/winehua/share/X11/xkb`)
- 首键自动 keyboard enter
...
| 2 | 路径计算错误 | `--prefix=/opt/winehua --libdir='${prefix}'` |
