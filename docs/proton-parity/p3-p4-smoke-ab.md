...
### 3.6 余量探针：把每帧绘制量翻 4 倍，帧时间不动

先试过"离屏渲染 + 不 Present"，**失败**：没有 present 就没人消费提交，
每帧 `Flush` 会把 vtest ring 灌满，测到第 60 帧就卡死（`p5f`）。

改用有界做法：保持一次 present 不变，只把每帧的 `DrawIndexed` 重复 K 次
（`WINEHUA_SMOKE_DRAW_LOOP`），跑 `p5g`（20 s/项，K=1 vs K=4）：

| 测试 | frames | avgMs | **renderMs** | presentMs |
| --- | --- | --- | --- | --- |
| `p5-k1-x86-fex` | 1573 | 12.3126 | 0.1717 | 11.7092 |
| `p5-k4-x86-fex` | 1591 | 12.2202 | 0.1680 | 11.6112 |
| `p5-k1-x86-wowbox64` | 1597 | 12.1294 | 0.1673 | 11.7653 |
| `p5-k4-x86-wowbox64` | 1590 | 12.1869 | 0.1800 | 11.7925 |
| `p5-k1-x64-native` | 1582 | 12.2351 | 0.0951 | 12.0167 |
| `p5-k4-x64-native` | 1586 | 12.2354 | 0.1029 | 12.0065 |
| `p5-k1-amd64-fex` | 1585 | 12.2340 | 0.1551 | 11.9326 |
| `p5-k4-amd64-fex` | 1597 | 12.1410 | 0.1369 | 11.8647 |

**绘制量翻 4 倍，`renderMs` 只动 0.01–0.02 ms，帧时间完全不动。**
折合每次绘制约 0.003 ms —— 说明 render 侧余量极大（数量级级别），
帧时间始终是显示节拍 + ~0.5–0.9 ms。

这也再次说明：**这个 cube 无论怎么加绘制量都问不出"后端快多少"**，
因为瓶颈根本不在被测量的那一段。要回答目标游戏的性能问题，只能上真实游戏，
或者构造一个把 Guest 提交/present 路径压满的负载。
