...
### 2.7 读了源码之后修正 §2.6 的推论

`thirdparty/wine/dlls/ntdll/unix/ohos_virtual.c` 里日志的开关不是环境变量，
而是静态计数器的**速率限制**：

```c
ohos_smc_log_count++;
if (ohos_smc_log_count > 64 && (ohos_smc_log_count % 32) != 0) return;
/* 注释: 不限流的 enter 行曾在 winedbg 循环时空转出 ~500MB wine_stderr */
```

据此反推本节会话的真实故障量级：

```text
观测 [SMC] 行 2014/2034 → 实际被记录的故障 ≈ 1017 次（每次故障 2 行）
其中前 64 次全记录, 之后每 32 次记录 1 次
=> 总故障数 ≈ 64 + (1017 - 64) × 32 ≈ 30,560 次
```

**两个修正：**

1. **"关掉 SMC 日志"不是有效的优化手段。** 日志已经按 1/32 限流，
   记录量只占实际故障的 ~3%；真正花时间的是那 ~3 万次故障本身
   （信号进入 + sigchain 判定 + 必要时转 Wine SEH），不是 `write(2)`。
   而且该文件里**没有**开关环境变量（只有 `WINEHUA_DUMP_PHDR`），
   要做 A/B 得改 Wine 源码 + 重编，收益却很小。**我上一轮把这条列为待做对照是错的。**
2. **记录的故障里约 65% 是 `not_mine`**（664/1017）—— 即大多数故障不是
   WineHua 的 SMC 处理对象，而被转交 Wine SEH。这说明要把 SMC 成本降下来，
   方向是减少"进信号 → 判定 → 转发"这条链本身的开销，而不是调日志。
