...
| OHOS-12 | FEX 正式统计 | G1 | **`pass_new`（创建）/ 读取待做** | `FEX_Config.json` + `share/fex-emu/Config.json` + `FEX_APP_CONFIG_LOCATION` | 真机确认 SHM 创建成功（`name/map/max/base` 齐全）；读取需 app 内工具（`/dev/shm` hdc 不可读） |
