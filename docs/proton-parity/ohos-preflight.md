...
| OHOS-12 | FEX 正式统计 | G1 | **`pass_new`** | `FEX_Config.json` + `FEX_APP_CONFIG_LOCATION`；PE 侧 dump 到 `C:\windows\temp\winehua_fex_probe.txt` | 真机读到统计头：`version=2 app_type=3 slot_size=80 capacity=4096 fex_version=FEX-2604-99-g86ff33b`，线程槽 `slot_off=64/144` 链表正确。扩容/退出清理未触发 |
